...
if (isset($_POST['btn_save'])) {
    ...
    $su = mysqli_query($con, "SELECT * from tblpwd");
        ...
        if ($name != "") {
            if (($imagetype == "image/jpeg" || $imagetype == "image/png" || $imagetype == "image/bmp") && $size <= 2048000) {
                if (move_uploaded_file($temp, 'image/' . $image)) {
                    ...
                    $update_query = mysqli_query($con, "UPDATE tblpwd set 
            ...
            $chk_image = mysqli_query($con, "SELECT * from tblpwd where id = '" . $_POST['hidden_id'] . "' ");
...
if (isset($_POST['btn_restore'])) {
    if (isset($_POST['chk_restore'])) {
        foreach ($_POST['chk_restore'] as $value) {
            $delete_query = mysqli_query($con, "UPDATE from tblpwd set statRes=0 where id = '$value' ") or die('Error: ' . mysqli_error($con));
...
if (isset($_POST['btn_delete_update'])) {
    if (isset($_POST['deleteItem'])) {
        foreach ($_POST['deleteItem'] as $value) {
            $delete_query = mysqli_query($con, "UPDATE tblpwd SET statRes = 1 WHERE id = '$value' ") or die('Error: ' . mysqli_error($con));
            if ($delete_query) {
                $_SESSION['delete'] = 1;
                header("location: " . $_SERVER['REQUEST_URI']);
                exit;
...
if (isset($_POST['btn_delete'])) {
    if (isset($_POST['chk_delete'])) {
        foreach ($_POST['chk_delete'] as $value) {
            $delete_query = mysqli_query($con, "DELETE FROM tblpwd WHERE id = '$value' ") or die('Error: ' . mysqli_error($con));
            if ($delete_query) {
                $_SESSION['delete'] = 1;
                header("location: " . $_SERVER['REQUEST_URI']);
