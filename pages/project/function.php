    ...
    if (isset($_FILES['files'])) {
        foreach ($_FILES['files']['tmp_name'] as $key => $tmp_name) {
            ...
            if (move_uploaded_file($tmp_name, $target)) {
                mysqli_query($con, "INSERT INTO tblprojectphoto (projectid,filename) 
...
if (isset($_POST['btn_save'])) {
    ...
    $update_query = mysqli_query($con, "UPDATE tblproject set dateofproject = '" . $txt_edit_doc . "', project = '" . $txt_edit_act . "', description = '" . $txt_edit_desc . "',  = '" . $txt_edit_ . "' where id = '" . $txt_id . "' ") or die('Error: ' . mysqli_error($con));
...
if (isset($_POST['btn_delete'])) {
    if (isset($_POST['chk_delete'])) {
        foreach ($_POST['chk_delete'] as $value) {
            $delete_query = mysqli_query($con, "DELETE from tblproject where id = '$value' ") or die('Error: ' . mysqli_error($con));
...
if (isset($_POST['btn_addimage'])) {
    ...
    if (isset($_FILES['photos'])) {
        foreach ($_FILES['photos']['tmp_name'] as $key => $tmp_name) {
            ...
            if (move_uploaded_file($tmp_name, $target)) {
                $query = mysqli_query($con, "INSERT INTO tblprojectphoto (projectid,filename) 
                    values ('$id', '" . $name . "')") or die('Error: ' . mysqli_error($con));
                if ($query == true) {
                    $_SESSION['added'] = 1;
                    header("location: " . $_SERVER['REQUEST_URI']);
...
if (isset($_POST['btn_remove'])) {
    if (isset($_POST['chk_deletephoto'])) {
        foreach ($_POST['chk_deletephoto'] as $value) {
            $delete_query = mysqli_query($con, "DELETE from tblprojectphoto where id = '$value' ") or die('Error: ' . mysqli_error($con));
